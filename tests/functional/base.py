...
class NMAPPluginTestClass(NMAPPlugin):
    __test__ = False
    ...
    def spawn(self, executable_path, arguments):
        p = Popen([executable_path] + arguments, stdout=PIPE, stderr=PIPE)
        stdout, stderr = p.communicate()

        self.do_process_stdout(stdout)
        self.do_process_stderr(stderr)

        self.do_process_ended(p.returncode)
